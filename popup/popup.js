...
if (tabs && tabs.length > 0) {
  ...
  easyReadTools.getStorageJsonData(easyReadTools.keyChainGenerate([easyReadTools.ALL_RECORDS_NAME, pageUrl]), (readedPage) => {
      if (readedPage) {
...
btnReadedAndRemove.addEventListener('click', async () => {
  ...
  if (tabs && tabs.length > 0) {
    ...
    easyReadTools.removeStorageJsonData(easyReadTools.keyChainGenerate([easyReadTools.ALL_RECORDS_NAME, pageUrl]), () => {
      document.getElementById("output").innerHTML = "The records of the page is removed.";
    });
  }
});
